// Connection.cpp — lifted from RUEEE/th06_multi_net (CC0), engine-agnostic.
...
#ifdef NETCODE_DEBUG_LOG
#include <cstdio>
void PrintError(const char* msg){ fprintf(stderr, "[net] %s\n", msg); }
void PrintError(const char* msg, int err){ fprintf(stderr, "[net] %s (WSA=%d)\n", msg, err); }
void PrintLog(const char* msg){ fprintf(stderr, "[net] %s\n", msg); }
#else
void PrintError(const char* msg){ (void)msg; }
void PrintError(const char* msg, int err){ (void)msg; (void)err; }
