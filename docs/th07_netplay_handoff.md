...
### §8av — kill the boss-fight lockstep re-align churn (the real "Parkinson's" + "drop", 2026-06-24)

§8at/§8au worked: the tester now gets PAST Cirno (desync moved to Letty). But two peers' logs (Wine, loopback 127.0.0.1, 2 instances) showed the real blocker: **3652 lockstep re-aligns + 2108 SEEDFORCEs in ~1242 frames**, `F1 SYNC wait=68ms` every frame (~14 fps), with **RNG self==peer throughout (NOT a desync)**. The cause is the START-BARRIER scene detector in `HookedSceneTick`: it re-zeros lockstep whenever `*(int*)(self+0x154)` changes. `self+0x154` is ZUN's top-level scene id (0..~12, incl. odd 1/5/9/11) ONLY when `self` is the scene task — but the per-frame task `self` we sample also lands on OTHER objects whose `+0x154` reads unrelated **even** values (logged: 2 and 6,10,22,46…68 — never an odd real scene). In-stage the real top-level scene is fixed (==2) and never changes mid-stage (the boss is within scene 2), so every in-stage "change" was spurious and fired a re-align + seed re-force EVERY frame → each re-align forces a fresh re-sync wait (the 68ms) → P2 input applied erratically ("Parkinson's") and the apparent drop. (`peer lost`/`HARD DESYNC` = 0.) Churn history: `new test logs` 9 re-aligns (died early), `newer test logs` 1805, this run 3652 — it scales with how deep you get, so it was newly *exposed* by getting to the boss, not newly caused.

**Fix:** honor a scene change only when **out of stage** (the menu FSM, where the detector always worked) or when the **in-stage flag itself flips** (`*ADDR_MODE_FLAGS>>2&1`, the real menu↔stage load that needs the clock re-zero); ignore in-stage scene-id noise. `s_netInStagePrev` tracks the flag; anchored at link-up. Both peers compute the flag from synced game state, so the menu→stage re-align still lands together. Build green, native test green (16/16). NEXT: tester re-test on this build — the boss churn / 68ms waits / Parkinson's should be gone; then judge the Letty replay desync on a CLEAN recording.

### §8aw — REVERT §8at/§8av: restore the working live foundation (2026-06-24)

The §8av re-test surfaced *new* live-run bugs (stage-2 CTD with nothing in the logs; **pause unpauses itself**, "as many SEEDFORCEs as failed pause attempts"). Stepping back with the user produced the crucial reframe: **there was a known-good live build, and we regressed past it.**

**The bisect (from the user's own test timeline):**
- `7b73eaa` **§8al** (06-22 22:40) — **last good build**: live netplay smooth, in-sync the whole run, clean stage transitions. (The focus-snapshot annoyance was *discovered* in this era but is a separate, always-present bit collision — fixed cleanly by §8au.)
- `a346e27` **§8at** (06-23 16:11) — **regression enters here**: "particle effect lags + EPOCH stuff in logs." Tester: *"the netcode doesn't agree with whatever you did to those particle effects."*

**What went wrong:** we traded the **working product (live co-op)** for a **diagnostic fix (replay-of-a-recording determinism)**. The Cirno/Letty desync only ever existed in *replay playback* — in a live run both peers execute identically, which is why **every** one of these captures shows `RNG self==peer`. §8at's cosmetic-RNG decouple ran on the live hot path (the `HookedRng32` 32-bit-wrapper detour did per-call return-address attribution + an `ebp`-walk + the decouple on **every** RNG draw under co-op, and the decouple wasn't even gated by `replay_trace`), perturbing the live particle/task stream → the lag, the EPOCH backlog, the connection drops. §8av was then a band-aid on the churn that the §8at era stirred up, and its in-stage-flag clause turned every pause (bit 2 going 1→0→1) into two `Nc_Reset`s → the self-unpause + one SEEDFORCE per pause.

**Revert (this build):**
- **Removed** the `FUN_004318d0` 32-bit-wrapper hook (`HookedRng32`) entirely — the §8ao/§8ar attribution + `ebp`-walk **and** the §8at cosmetic decouple. The 32-bit RNG path returns to native (zero co-op overhead).
- **Reverted** §8av's re-align gate back to the §8al original (`if (sceneId != s_netSceneId)`), dropping `s_netInStagePrev` and the in-stage-flag clause — i.e. `HookedSceneTick` is byte-identical to §8al again.
- **Kept §8au** (the snapshot `0x800` strip) — it fixes a real product bug and is a clean input-bit strip in the present path (no RNG, no task perturbation). This is the one live-path element beyond §8al; watch it on re-test.
- **Kept** the §8am per-frame histogram diagnostic (strictly better than §8al's per-draw I/O, and self-gated by `replay_trace`).

Net: the live path = **§8al + (§8au snapshot fix) + (gated §8am histogram)**. Build green (873,472 B), native test green (16/16). The replay-playback determinism fix is **shelved**; if revisited it must be done **off** the hot path (hook the three cosmetic functions `FUN_0041a370/a5a0/aa60` directly + flag them, not an `ebp`-walk per draw). NEXT: tester re-runs **live** on this build — expect the §8al-smooth run back (no particle lag, no EPOCH storm, no churn, pause holds); then re-judge the stage-2 CTD on this clean base before assuming it's anything new.

### §8ax — RETHINK via the EoSD clean-room decomp: the desync axis is the FRAMERATE/TIMESTEP MULTIPLIER, not the RNG (2026-06-24)

With the foundation stable, we re-examined *why* the replay-playback desync exists and why §8at failed, cross-referencing the EoSD clean-room decomp (`d:/PCB Co-op/th06_multi_net`, a netplay EoSD fork — same ZUN engine lineage). Findings:

- **§8as's consumer ID was correct but it was the canary, not the disease.** PCB `FUN_0041a370` *is* EoSD `EffectManager::EffectCallbackRandomSplash` — identical formula `(g_Rng.GetRandomF32ZeroToOne()*256-128)/12`, drawing from the shared `g_Rng`. §8at cut that one consumer's seed coupling, so the *first* visible divergence went quiet — but the desync only **moved** (Cirno→Letty) and the ±4 residual re-compounds. Classic symptom patch. Our own §8as note already said the trigger was "a frame-counter/state off by one," which we then ignored.
- **The real axis: the framerate/timestep multiplier.** Every game timer ticks through `Supervisor::TickTimer` / `ZunTimer::Increment`, and EoSD's code makes the rule explicit: when `framerateMultiplier <= 0.99` timers switch from clean integer `current++` to **fractional sub-frame accumulation** (`subFrame += effectiveFramerateMultiplier; if(>=1) current++`), and *all motion* advances by `pos += vel * effectiveFramerateMultiplier`. EoSD **guards replay determinism** by forcing `framerateMultiplier = 1.0f` and *refusing to record* under slowdown (`ReplayManager::RegisterChain`). PCB is byte-identical: `FUN_0043958d`=TickTimer, `FUN_00439401/FUN_004394c7`=ZunTimer::Inc/Dec, gate+scale = `DAT_00575ac8` (Supervisor `framerateMultiplier`, +0x178); the §8as splash velocity even multiplies by `DAT_00575ac8`.
- **Unified hypothesis:** a replay stores *inputs keyed by logic-frame + one seed* and assumes the sim advances **exactly one integer frame per logic frame**. A live **netplay** run does NOT hold a clean 1.0 — the lockstep waits (measured 68 ms stalls; Wine + 2 instances) push the game into slow/catch-up stepping, so during the **recording** timers tick fractionally and motion advances by a variable amount, while **playback** (smooth, 1.0) advances by exactly 1.0 → every timer-gated event (enemy fire, bullet cancel, item drop, particle spawn) crosses integer thresholds one frame off → seed phase shifts → bullets diverge. This explains: netplay-recordings-only (local holds 60 fps → record and playback both integer → identical), the always-"one-frame-off" signature, why seed-pinning matched the seed while objects drifted, and why §8at only relocated the desync. It supersedes the §8ad x87 theory (the "same inputs+seed, different evolution" axis was the timestep, not FP precision).
- **The EoSD-validated fix (if confirmed):** pin the netplay sim to `DAT_00575ac8 = 1.0` (clean integer timestep) — the lockstep wait delays *wall-clock* only, the logic step is always exactly one integer frame. One root fix at the source, replacing the whole seed-pin / RNG-decouple pile.

**Diagnostic shipped this build (read-only, gated by `replay_trace`):** `ReplayDetTrace` (coop_rdt_{rec,rpy}.csv) gains three columns — `mult` (`DAT_00575ac8`), `mult2` (`DAT_00575ac4`), `slow` (`DAT_00575adc` bit 5). **DECISIVE TEST:** one netplay record→playback with `replay_trace=1`; diff the `mult` column rec-vs-rpy by `rf`. If the **rec** stream ever shows `mult != 1.000` (or differs from rpy), the timestep is the desync axis → pin it. If `mult` is flat `1.0000` in both, the multiplier is ruled out cleanly and we pivot. (User confirms tester runs 60 fps, so the *config* multiplier is 1.0; any deviation would be the adaptive catch-up path engaging under Wine load.) Build green (873,472 B), native test green (16/16). See [[live-netplay-vs-replay-playback]] and the EoSD ref [[eosd-cleanroom-decomp]].

### §8az — the timestep hypothesis is REFUTED; pivot to the input-path axis (2026-06-24)

`§8ay` (3d4fb80) moved the `mult`/`mult2`/`slow` columns into the always-on NetTrace so a normal run captures the live-record timestep without `replay_trace`. The tester's clean §8ay capture settles §8ax: **`mult = 1.0000` on all 32,705 frames, both peers; `slow = 0` throughout.** The live netplay run advances at a clean integer timestep — identical to playback. **The framerate/timestep multiplier is NOT the desync axis** (it joins x87 precision, §8ad, as ruled out). Sample-point caveat is covered by `slow = 0` and the slow-mode funcs `FUN_00418eb0/fc0` being unreferenced.

That run was clean §8aw (5 re-aligns, 0 HARD DESYNC, 0 peer-lost) and **the replay desync moved LATER — to Letty (st1 boss) instead of Cirno (st1 midboss).** That strongly implies the §8as "cosmetic particle at rf698/Cirno" root-cause was an **artifact of the §8at-era live regression** (particle lag / churn / EPOCH corrupting the recording), not the true cause. So the genuine first divergence on a clean foundation is unknown and must be re-found.

**Pivot.** Two "physics" axes are now excluded (FP precision, timestep). The defining difference between a **netplay** recording (desyncs on playback) and a **local** recording (replays perfectly) is the **input path** — netplay applies inputs through input-delay + the wire + lockstep, playback feeds them directly. The next decisive diagnostic is a **clean §8aw rec-vs-rpy determinism diff**: one netplay run at `replay_trace=1` (writes `coop_rdt_rec.csv`, saves the `.rpy`), then replay that `.rpy` at `replay_trace=1` (writes `coop_rdt_rpy.csv`); diff by `rf`. The **first column to diverge** — `input` / `seed` / `counter` / `p1`/`p2` position — names the axis: an `input` mismatch = the recording isn't input-faithful; identical `input` with diverging `seed`/`counter`/`position` = a sim-execution difference given faithful inputs. (`mult` is in the rdt too, expected 1.0 on both. The `.rpy` body is ZUN-compressed, so this can't be parsed offline — the in-game capture is required.) Also noted in the live trace: the RNG `counter` oscillates ±22/±44 per frame in-stage (the §8aq transient) — worth confirming on the clean diff.

### §8bb — FIX the stage-transition desync (P2 respawn not reproduced on playback) + in-stage attribution (2026-06-25)

The first clean §8ba multi-stage capture (rec-vs-rpy, draws counter) localized the desync precisely. **Stage 1 is fully deterministic** (162,443 draws identical). The fork is at the **stage-1→2 boundary**: with the +1 rf alignment the `input` matches perfectly (recording is faithful) and the seed matches only because §8aj pins it — but the new `draws` delta forks at stage-2 rf0, and the cause is a **P2 lifecycle asymmetry**. On the live record P2 **de-spawns at the boundary and auto-respawns ~3 s into the next stage** (`s_p2` null rf1→240, back at rf241 — user confirms this is intended, resources carry over); on playback **P2 is never absent** — it carries the stage-1 P2 straight through. So playback runs P2's per-frame update (drawing RNG) for ~240 frames the live run didn't → the stage-2 sim forks. Root: the de-spawn lives in `HookedFrameTask` (the RECORD task `FUN_00442cd0`), which ZUN does **not** run during playback (it runs `FUN_00442ee0`), so it never fired on the playback side.

This is **co-op- and stage-transition-specific**, which is why it was never seen before: vanilla single-player replays don't have P2, and the "local co-op replays work" tests were all on **Phantasm — one stage, no transitions** (user confirmed), so they never exercised a boundary.

**Fix (§8bb):** on the playback path (in `HookedUpdate`, which runs on both record and playback and sits after `DespawnP2`), detect the per-stage replay-frame **reset** (`*ADDR_REPLAY_MGR` rf going backwards) and, for a co-op replay with P2 present, **`DespawnP2()` + re-arm the auto-spawn** — exactly what the record task does — so P2's absent-then-respawn window matches the live run frame-for-frame. Detection is after the det-trace sample so rf0 still shows the carried P2 like the record's rf0; own tracker `s_rpyStageFrame`; record path untouched (no live regression).

**Caveat:** does NOT explain the **during-stage** desyncs — the §8az capture's playback died *inside stage 1* with no P2 respawn anywhere near it. That is a separate, intermittent fork (likely the "one-frame-off event that usually heals but sometimes doesn't"). §8ba's stage 1 happened to be the healing case. Not solved; the `draws` counter will pinpoint it when it recurs.

**In-stage diagnostic added:** (1) raised the rngcall-histogram rf cap 4000→60000 so it covers stage 2+; (2) **restored the §8ar 32-bit-wrapper consumer attribution** (`HookedRng32`) so the histogram names the REAL consumer of wrapper-routed draws (the bulk), not the wrapper site — **read-only, NO §8at decouple**, and **enabled ONLY under `replay_trace`** (the product hot path stays byte-identical to §8aw — the trampoline isn't even installed otherwise). So when the `draws` counter flags an in-stage fork frame, the rngcall CSV names the function that drew the extra rands.

Build green (874,496 B), native test green (16/16). NEXT: tester re-captures rec-vs-rpy on this build — expect the stage-2 boundary `draws` delta to drop to ~0 (P2 window now matches); if a during-stage fork appears, the draws delta + the restored attribution name the frame and the consumer.

### §8bc — the in-stage replay fork ROOT, and the EoSD answer for why their 2P replays "just work" (2026-06-25)

The §8bb capture (89d1891) delivered exactly the in-stage fork we instrumented for (tester: "desyncs in the latter half of stage 1"). Analysis (rec-vs-rpy, +1 rf alignment, the `.rpy` came from rec block 1, `rpy_rf = rec_rf + 1`):

**Meta-finding — the §8aj seed-pin is a USELESS oracle.** Across all 8001 playback frames the `seed` column **NEVER** mismatches (the per-frame re-pin forces it). Yet the §8ba `draws` counter shows the truth: cumulative draws hold steady at the `+8` init offset through rf1164, then **permanently drift negative**, reaching **−2605 by rf8000** — playback persistently *under-draws*. This retroactively explains every "matched N/N frames but still desynced" result: seed-equality is blind; **the `draws` counter is the only valid replay-desync oracle.**

**Onset pinned — rf1163, first divergent consumer = `FUN_004012b0`.** At the very first divergent frame the rngcall histogram (restored §8bb attribution) shows record drew 4 rands from `0x4012cc`+`0x401306` (inside `FUN_004012b0`, called ×2) that playback drew **0**; the other −4 of the −8 step is invisible direct-16-bit draws from the same event. `FUN_004012b0` draws 2 wrapper rands and writes a **self-rescheduling "next-fire-time" field** (`+0x3c/+0x98 = rand%100000 + 0x198f`, `+0x9614 = float(next)`). It is NOT the cosmetic splash `FUN_0041a370` (the §8as/§8at suspect) — the splash wobbles come *after*, as downstream amplification. **Crucially, at rf1163 both players are in IDENTICAL positions** (P2 pos doesn't fork until rf4844, P1 until rf6222 — long after, as the RNG-poisoned bullets cause different deaths). So the trigger is a **scheduler/phase drift**, not a player-state difference: same seed + same input + same positions, yet the rescheduler fired live and not in playback.

**Why (the EoSD answer).** The EoSD coop decomp (`th06_multi_net`) makes P2 a **first-class statically-allocated second player** — `DIFFABLE_STATIC(Player, g_Player2)`, a permanent engine-native object in the calc/draw chains, updated every frame, reading its input from the `*2` bits (`SHOOT2/FOCUS2/DIRECTION2`) of the single global input word. Its replay records that **one merged word** (`TH_BUTTON_REPLAY_CAPTURE` = both players' buttons) as a frame-keyed edge list + **one seed per stage** (`g_Rng.Initialize(replayData->randomSeed)` in `AddedCallbackDemo`), and playback just feeds the merged word back into the **fully deterministic two-player sim** — *no networking, no per-frame seed re-pin, no separately-injected entity*. 2P replays are then free: `seed + merged input → identical sim`. **PCB-Coop's P2 is the opposite: a runtime memcpy CLONE** (`753272 bytes`) injected ~3 s into each stage by our mod's lifecycle logic. The clone's RNG-consuming actions and self-rescheduling timers are a **side channel the replay never captures** — so `seed + merged input` does *not* fully determine the sim, which is exactly why PCB needed the §8ah–§8aj seed-pin band-aids and still forks. The rf1163 `FUN_004012b0` drift is the first symptom of that architectural gap. We can't statically allocate a second `Player` in a binary mod (fixed arrays — see eosd-cleanroom-decomp 2nd example), but the actionable corollary holds: any consumer that draws from the **shared** seed on a path the clone perturbs asymmetrically must be made deterministic-from-input — either decoupled to a private seed (the §8at thesis, done *symmetrically* on record+playback, with `HookedSceneTick` hardened so it can't churn) for genuinely **cosmetic** schedulers, or driven purely by the recorded merged input for **gameplay** ones.

**§8bc logging added:** hook `FUN_004012b0` (read-only, `replay_trace`-gated, never laid in a product run) and tally its **caller** — `__builtin_return_address(0)` in a MinHook detour is the true call site — into the per-frame histogram, tagged `0x80000000` (reads as `8xxxxxxx` vs a `00xxxxxx` RNG consumer). The next capture's rec-vs-rpy diff at rf1163 names *which* of the ~25 call sites fires live but not in playback → settles cosmetic-vs-gameplay and picks the fix. Build green (874,496 B), native test green (16/16).

### §8bd — the §8bc call-site lands on ITEM COLLECTION; the fork is non-RNG clone state, not a draw drift (2026-06-25)

The §8bc capture (a full stage1→6 live netplay run + a stage-1 replay that desynced) named the call site. The **first and root** divergent `FUN_004012b0` caller is **`0x00432620` = inside `FUN_004325e0`**, called from PCB's **point/power-item collection handler `FUN_00432990`** ([PCBdecomp.c:20329](../PCBdecomp.c)/cases at 20448–20639: `FUN_0048b8a0` rolls the bonus tier, `DAT_00626278+4`=score, `+0x24`=point-items, `+0x2c/0x30`=extend thresholds). So `FUN_004012b0` here fires **on item pickup** — gameplay scoring RNG, not cosmetic. The four other divergent callers (`0x43edd3`, `0x43b790`, `0x42d60d`, `0x440dd0`) appear **only in playback and later** = downstream debris.

**The decisive evidence (this capture):** at the first fork **rf1037, playback collected ONE extra item-bonus while both players are byte-identical in position** (P1 doesn't diverge until rf2622, P2 until rf4461 — ~1600 frames *later*), **and the RNG histogram shows NO consumer diff anywhere before rf1037** — the shared draw stream is identical up to the fork. Identical RNG + identical players ⇒ the extra collect is **not** an RNG/physics drift; it is a **non-RNG state asymmetry in the item-collection path** — a clone-side difference (or a +1-offset double-count) that first *consumes* RNG when the extra bonus rolls, which then poisons the shared seed phase and cascades into bullets, then players. This is the clone-P2 side channel (§8bc / EoSD) made concrete: **the players are deterministic, but item collection is not.** It also means "more RNG attribution" is the wrong next instrument — there is no earlier RNG fork to find.

**§8bd instrument (decided by the user: option 1 then option 3).** Two read-only, `replay_trace`-gated additions (product run with `replay_trace=0` is behavior-identical — a flag-test no-op):
1. **rdt columns `score,pitems`** (`*(DAT_00626278+4)`, `+0x24`) — name the exact frame *and kind* of the collection fork, and tell a benign **+1-offset double-count** (a constant +1 that never grows) from a **real divergence** (a growing delta).
2. **Per-collect event log** in `HookedCollectOverlap` (the `0x43e4e0` P1-then-P2(clone) collection seam): `COLLECT rf=… by=P1/P2 item=(x,y)`. A rec-vs-rpy diff of the `COLLECT` lines names the rf1037 extra collect — **who** (is it a P2-clone-only collect? → option 3 root) and **which item**.

(Item-*spawn* logging skipped deliberately: the identical pre-fork RNG stream already proves item spawns are deterministic.) Build green (875,008 B), native test green (16/16). NEXT: tester re-captures rec-vs-rpy on this build; the `pitems`/`score` columns + `COLLECT` diff pinpoint the collection asymmetry → feeds option 3 (which clone field gates it).

### §8be — the fork is a CHERRY divergence (point-item value), not a clone position/collect miss (2026-06-25)

The §8bd capture (live stage1→6 + a stage-1 replay that desynced) settled it. In the clean playback (the rec CSV holds the multi-stage live run; the rpy holds two watch runs — analyse **block 0**, align `rpy_rf = rec_rf + 1`):

- **Both players are byte-identical for the ENTIRE stage** (no P1 *or* P2 position divergence) and **score/pitems/draws are byte-identical until physical frame 854.** The clone is not drifting and is not missing collects (the early playback `COLLECT` lines just weren't all logged, but the score climbed in lockstep: 438→495→713→988).
- **The fork is one point item.** At frame 854 the **same item (354,203.3)**, collected by **P1**, on the **same physical frame**: record scores **+1** (`pitems` stays 0), playback scores **+4250** and `pitems`→1. Same item, same frame, same positions, identical RNG up to that instant — yet record treats it as ~nothing and playback as a **MAX-value** point item. The draw count forks −4 right there (the value computation) and cascades.

**Why (user + `docs/th07_cherry_determinism.md` §0).** Point-item value = `f(Cherry, POC-proximity)`, capped by Cherry Max; **during a border all items collect at MAX**; Cherry fills from **shots/graze** and is the handoff's **#1 determinism risk**. `pitems` was 0 until rf854, so **rf854 is the FIRST point item collected** — which is exactly when a cherry that diverged *silently earlier* (no point items before it to reveal the difference) first shows up in score. So two byte-identical states (RNG + both players) produced different collection values because the **hidden cherry state had already diverged** — `+4250 = MAX` means playback saw either a higher Cherry or an active border at that item. This is the clone side channel (option 3) made concrete, in the cherry layer rather than position.

**§8be instrument:** rdt gains `cherry,cmax,cplus,cbase,p1st,p2st` — the three cherry deltas (`*0x62f88c/0x62f888/0x62f890 − cherry_base@(*0x626278+0x88)`) + both player STATE bytes (state 4 = border active). Read-only, `replay_trace`-gated (product run = flag-test no-op). The next rec-vs-rpy diff names the exact frame **Cherry / Cherry+ / border** forks — *before* rf854 — which points at the cherry-GAIN source (P2-clone shots/graze accounting) to fix. Build green (875,008 B), native test green (16/16).
